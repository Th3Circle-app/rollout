  ...
  return (
    <div className="relative">
      ...
      <div className="absolute inset-0 z-40 flex items-center justify-center p-6">
        <div className="panel w-full max-w-sm rounded-3xl p-8 text-center backdrop-blur">
          <div className="mx-auto mb-5 flex size-12 items-center justify-center rounded-2xl bg-violet-500/15 border border-violet-500/25">
            <Lock className="size-5 text-violet-500" />
          </div>
          <div className="kicker kicker-center justify-center mb-3">
            {tier === "studio" ? "Rollout Studio" : "Rollout Artist"}
          </div>
          <h2 className="page-title text-[26px]">{feature}</h2>
          <p className="mt-3 text-[15px] leading-relaxed text-[#9A96AD]">{blurb}</p>
          <Button
            onClick={() => openUpgrade(feature)}
            className="btn-primary mt-6 w-full gap-2 rounded-xl py-5 font-semibold text-white"
          >
